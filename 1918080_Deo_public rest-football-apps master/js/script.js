const ApiKey = "3d1be21c7f6c41f3a8f1ccd952fbc265";
...
/*function showTeamInfo(id){
    ...
    contents.innerHTML="detail team dengan id" + url;
    fetch(teamEndPoin, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            console.log(resJson.teams);
            ...
            resJson.teams.forEach(team => {
                ...
                <li class="collection-item avatar">
                    <img src="${team.crestUrl}" alt="" class="circle">
                    <span class="title">${team.name}</span>
                    <p>nama: ${team.shortname} <br>
                       alamat: ${team.address} <br>
                       nomer hp: ${team.phone} <br>
                       email: ${team.email} <br>
                       warna club:${team.clubColors}
                    </p>
                </li>
                `
            });
...
function showTeamInfo(id){
    let url = baseUrl + "teams/" + id;
    contents.innerHTML= `detail team dengan id <br>
                        nama: ${url.shortname} <br>
                        alamat: ${url.address} <br>
                        nomer hp: ${url.phone} <br>
                        email: ${url.email} <br>
                        warna club:${url.clubColors} <br>
                        link data team bola : ` + url;
}
